...
inline std::vector<uint8_t> hash256(const void* data, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) padded[i] = msg[i];
    padded[size] = 0x80;

    size_t pad_end = size + 1;
    while ((pad_end + 8) % 64 != 0) { padded.push_back(0); ++pad_end; }
    for (int i = 0; i < 8; ++i)
        padded.push_back(static_cast<uint8_t>((bit_len >> (56 - i * 8)) & 0xFF));
    ...
    for (int i = 0; i < 8; ++i) {
        result[i * 4 + 0] = static_cast<uint8_t>((h[i] >> 24) & 0xFF);
...
inline std::string hash256_hex_string(const void* data, size_t size) {
    auto hash = hash256(data, size);
...
inline std::string hash256_hex_string(const std::vector<uint8_t>& data) {
    return hash256_hex_string(data.data(), data.size());
}
